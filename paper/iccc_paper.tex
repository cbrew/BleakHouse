...
Table~\ref{tab:convergence_detail} shows the provision-dimension
profiles of passages selected by the two reading strategies, averaged
across 28 novel$\times$panel pairs.

\begin{table}[t]
\centering
\small
\caption{Fraction of selected passages rated `strong' per provision
dimension, by reading strategy.}
\label{tab:convergence_detail}
\begin{tabular}{lrrr}
\toprule
Dimension & Synoptic & Analytical & $\Delta$ \\
\midrule
...
atmosphere\_setting & 19.1\% & 18.3\% & +0.9 \\
narrative\_technique & 90.1\% & 92.5\% & $-$2.4 \\
\bottomrule
\end{tabular}
\end{table}

The two strategies have systematically different provision profiles
(synoptic favours character and plot; analytical favours social
critique and thematic depth) yet produce convergent expert airtime:
Hartley 32--37\%, Blackstone 30--37\%, Woodcourt 29--34\% across all
15 novels.  The standard deviation per expert across novels is less
than 2~pp.
...
selected.  Despite this near-disjointness, the resulting discussions
are framework-convergent: the interpretive lens, not the reading
strategy, determines the character of the output.
