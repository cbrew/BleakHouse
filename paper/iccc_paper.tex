...
\pdfinfo{
/Title (The Reader Makes the Reading)
/Subject (Proceedings of ICCC)
/Author (Anonymous)}

\title{The Reader Makes the Reading:\\
Framework Dominance and Pastiche in LLM-Generated Literary Discussion}
\author{Anonymous}
...
\emph{framework dominance}: two algorithmically distinct reading
...
critical perspectives and textual affordances, where passage exploration is free and script generation costs only
LLM tokens.  Removing textual grounding reveals that ungrounded
...
A caveat is needed.  Framework dominance is not a single mechanism but
the cumulative effect of a pipeline that consistently privileges the
expert's identity at every stage: enrichment dimensions are defined
in terms that match expert demand profiles; the passage selector
optimises for expert-demand satisfaction; host preparation
pre-interviews each expert individually and plans questions that
target specific experts; and the script-generation prompt explicitly
instructs speakers to maintain their distinctive perspectives and
react to each other's claims.  Each stage reinforces the framework.
Whether the finding reflects a deep property of interpretive discourse
or merely the pipeline's engineering is an open question---but the
engineering is the mechanism, and we should name it honestly.
...
Miss Marjoribanks & Oliphant & 3 & 94\,pp \\
North and South & Gaskell & 2 & 95\,pp \\
...
Passage-level exploration incurs no LLM cost (the solver runs locally
in $<$100ms).  The expensive step---script generation, which costs
$\sim$100K Sonnet tokens per episode---is deferred until the editor
has identified a promising configuration.
...
framework dominance finding provides an empirical answer: the persona
...
First, the \emph{framework dominance finding still holds}.  Despite radically
...
The framework dominance finding makes one thing obvious: the personas and
...
the framework dominance finding: since the interpretive framework determines
...
predictable---but they are not.  The framework dominance finding was
...
for the reason the framework dominance finding demonstrates: the prose is
