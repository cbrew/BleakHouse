...
scripts partially converge.  Table~\ref{tab:material} shows passage-level
...
The script generation prompt encodes measurable conversational design
features.  Across all 186 episodes:
...
Quote pattern compliance is now 100\%: every quote utterance carries a
passage reference or reading mode annotation.  Reactive markers (59/ep)
indicate genuine conversational engagement between experts.
...
\textbf{H1 (manipulability)}: confirmed.  The demand-profile mechanism
allows systematic, predictable control over passage selection.
Changing demand vectors reshapes the passage set in ways that are
legible as flow changes and impossible to replicate via retrieval
parameter tuning.  The interest-based discard cost further ensures
that high-quality passages are preferentially retained.
