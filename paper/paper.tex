...
the host, Zephyr for Hartley, Sadaltager for Blackstone) with an accent
...
The rendering pipeline is pluggable at the profile level: a
\texttt{TTSProfile} abstraction selects the model, voice mapping,
pause scaling, and prompt builder.  The results reported here use the
\textit{classic} profile (Gemini 2.5 TTS, short stage-direction
prompts) uniformly across all conditions.  As an alternative render of
one panel, we have also produced a \textit{trevelyan\_v2} rendering of
the Panel~B \textit{Bleak House} episode using
\texttt{gemini-3.1-flash-tts-preview} and the six-strategy prompting
scheme from Google's speech-generation
documentation---Audio Profile, Scene, Director's Notes, Sample
Context, Transcript, and audio-tag hints.  The alternative profile is
not part of the quantitative comparison; it is available on the demo
site\footnote{\url{https://bleakhouse-demo.fly.dev/examples}} and
serves as a qualitative check that the Phase~3 output schema is
stable under a different rendering backend.
