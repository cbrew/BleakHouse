...
full inference call.  We test this by generating multi-voice literary
discussions of Dickens's \textit{Bleak House} across five passage
selection conditions and 20 expert panel configurations (100 episodes),
then replicate the key findings on four additional novels (96 episodes).
...
generation) and stable expert identity signatures, replicates across all
...
We test these hypotheses by generating multi-voice literary discussions
of Dickens's \textit{Bleak House} \citep{dickens1853bleak}, comparing
five passage selection conditions across all $\binom{6}{3} = 20$
possible three-expert panels drawn from six personas.  The comparison
reveals that \emph{having} passages matters far more than \emph{how}
they are selected for downstream script quality---but the transport
formulation uniquely enables systematic editorial exploration.
...
Each of 6{,}916 passages from \textit{Bleak House} (67 chapters) is
annotated by an LLM (Haiku) with 20 structured fields: an interest
score (0--5), seven provision dimensions
...
enrichment cost: ${\sim}$8M Haiku tokens plus ${\sim}$1.4M OpenAI
embedding tokens, amortised across all runs at zero marginal cost.
...
All $\binom{6}{3} = 20$ possible three-expert panels, each run under
all five conditions: 100 episodes total for \textit{Bleak House}.
A cross-novel replication (Section~\ref{sec:generalization}) adds four
novels with 8 panels $\times$ 3 conditions = 96 episodes.
...
against 23 characters), character entropy
...
cosine), quote verification (fuzzy 5-word subsequence matching against
source text), and material similarity (Jaccard, Jensen-Shannon
divergence \citep{lin1991divergence}, maximum mean discrepancy
...
\begin{table}[t]
\caption{Token budget by model class.  Enrichment is computed once;
  per-run costs are dominated by script generation.}
\label{tab:cost}
\begin{tabular}{lrr}
\toprule
Component & Haiku & Sonnet \\
\midrule
Enrichment (once)   & 8.0M & --- \\
Segment design (/run)  & 2K & --- \\
Curation (/run, embed only) & --- & 15K \\
Script generation (/run) & --- & 102K \\
\midrule
Project total ($\sim$390 runs) & 8.8M & 39.5M \\
\bottomrule
\end{tabular}
\end{table}

The enrichment layer's 8M Haiku tokens are amortised across all runs.
Adding a new panel costs $\sim$300K Sonnet tokens (3 conditions
$\times$ $\sim$100K each).
...
Table~\ref{tab:aggregate} shows that the five conditions produce
comparable scripts on surface metrics.

\begin{table}[t]
\caption{Aggregate metrics across 20 panels per condition.}
\label{tab:aggregate}
...
alone.  These findings confirm that selection method has modest
impact on aggregate quality---the persona layer dominates.
...
The gap between no-passages (38\%) and any grounded condition
($\geq$78\%) dwarfs the gap between grounding methods.  Passage
grounding prevents confabulation; the method of grounding is
secondary.
...
Transport and embedding select almost entirely different passages
(Table~\ref{tab:material}), yet scripts partially converge.

\begin{table}[t]
\caption{Material disjointness (top) vs script convergence (bottom),
  transport $\leftrightarrow$ embedding, per-expert averages.}
\label{tab:material}
\begin{tabular}{lr}
\toprule
...
discussing different passages about \textit{Bleak House} arrives at
...
design features.  Across 233 episodes (114{,}392 utterances):

\begin{table}[t]
\caption{Conversational design measurements (233 episodes).}
...
\section{Cross-Novel Generalization}
\label{sec:generalization}

The preceding results derive from a single novel.  To test whether the
pipeline's properties generalize, we run the same system on four
additional novels spanning different authors, eras, and literary
traditions: Dickens's \textit{Our Mutual Friend} (1865, 5{,}775
passages), Eliot's \textit{The Mill on the Floss} (1860, 4{,}654),
Gaskell's \textit{North and South} (1855, 2{,}942), and Forster's
\textit{A Passage to India} (1924, 2{,}556).  Each novel is processed
through the full enrichment pipeline, producing per-passage provision
scores, character annotations, and contextual embeddings.  We then run
all three conditions (transport, embedding, no-passages) with eight
expert panel compositions, yielding 96 episodes (4 novels $\times$ 3
conditions $\times$ 8 panels).

\subsection{Grounding Gap Replicates}
\label{sec:xnovel-grounding}

\begin{table}[t]
\caption{Quote verification rates by novel and condition.  The
  grounding gap (grounded mean minus no-passages) is large and
  consistent across all four novels.}
\label{tab:xnovel-quotes}
\begin{tabular}{lrrrc}
\toprule
...
\midrule
\textit{Mean}      & 62\% & 58\% & 10\% & +50 \\
\bottomrule
\end{tabular}
\end{table}

The grounding gap---the difference in quote verification between
passage-grounded and ungrounded conditions---replicates across all four
novels (Table~\ref{tab:xnovel-quotes}).  Grounded conditions verify at
58--62\%; the no-passages baseline at 10\%.  The gap (+50 percentage
points on average) is comparable to the single-novel result.  Forster's
novel shows the highest no-passages rate (20\%), consistent with the
prediction that better-known texts produce fewer confabulations even
without grounding---but the gap remains large.


\subsection{Convergence Paradox Replicates}
\label{sec:xnovel-convergence}

\begin{table}[t]
\caption{Material disjointness vs.\ script convergence across novels.
  Passage Jaccard is near-zero everywhere; vocabulary cosine is
  consistently moderate.}
\label{tab:xnovel-convergence}
\begin{tabular}{lrrrr}
\toprule
...
The convergence paradox holds for every novel tested
(Table~\ref{tab:xnovel-convergence}).  Transport and embedding select
nearly disjoint passages (Jaccard 0.024--0.085) yet produce scripts with
moderate vocabulary overlap (cosine 0.632--0.683).  The convergence force
is the expert persona: the same expert discussing different passages
about the same novel arrives at overlapping vocabulary because the
persona constrains what the expert \emph{says about} the material.


\subsection{Expert Identity Is Stable Across Novels}
\label{sec:xnovel-identity}

\begin{table}[t]
\caption{Expert airtime (\% of total expert words, transport condition)
  is remarkably stable across novels.  Blackstone consistently
  dominates; Trevelyan is consistently most concise.}
...
Expert personas maintain stable behavioral signatures across novels
(Table~\ref{tab:xnovel-airtime}).  Airtime proportions vary by at most
1.4 percentage points (SD) across the four novels.  Cross-novel
...
fingerprint.  Sentence-type profiles are similarly stable: Trevelyan
consistently shows the highest quote\_reading proportion (10.6\%),
Blackstone the lowest (6.4\%), regardless of novel.
...
cross-novel generalization study (Section~\ref{sec:generalization})
addresses the single-novel limitation but uses the same automated
metrics; human evaluation across novels remains future work.
