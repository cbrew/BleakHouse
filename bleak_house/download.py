...
# a generic gutenberg downloader belongs here.
QUIXOTE = "https://www.gutenberg.org/cache/epub/5921/pg5921-h.zip"
...
def plaintext(url: str = URL) -> Dict[str, Any]:
    ...
    return {"source": "gutenberg", "text": response.text}
...
def html_paths(url: str = HTML_ZIP_URL) -> Dict[str, Path]:
    ...
    z.extractall("data")
    names = {f: Path(f"data/{f}") for f in z.namelist()}
    return names
...
def chapters(html_paths: Dict[str, Path]) -> Dict[str, str]:
    ...
    for name, path in html_paths.items():
        if name.endswith(".html"):
            ...
            book_title = tree.xpath("//head/title")[0].text
    ...
    assert n_keys == 1, f"Expected 1 key, got {n_keys}"
