...
For each (novel, panel) coordinate, pick exactly one run with audio. The
ranking carries forward the policy from webapp/consumer.py:_rank_key
plus a new short-preferred-over-long key (the user-invisible 'length'
axis). Returns one Episode per (novel, panel) bucket; coordinates with
no audio-bearing run are dropped.
"""
...
def list_canonical_episodes() -> list[Episode]:
    """Return one Episode per (novel, panel) pair that has audio.
...
def canonical_run_for(novel: str, panel: str) -> Episode | None:
    """Return the canonical Episode for (novel, panel), or None."""
    for ep in list_canonical_episodes():
        if ep.novel == novel and ep.panel == panel:
            return ep
    return None
