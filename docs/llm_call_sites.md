...
| File:line | Call | Model | Role |
|---|---|---|---|
| `enrichment/submit_context_batch.py` | batch submit | `claude-haiku-4-5-20251001` | Passage contextualisation |
| `enrichment/collect_context_batch.py` | batch collect | — | Passage contextualisation |
| `enrichment/generate_contexts.py:155` | `.messages.create` with prompt caching | `claude-haiku-4-5-20251001` | Alternate synchronous path for context generation |
...
|---|---|---|---|
| Anthropic | ~12 call sites across 10 files (main pipeline, batch utilities, Phase 2.5) | 2 (`test_single.py`, `generate_contexts.py` sync path) | 1 (`metrics.py`) |
