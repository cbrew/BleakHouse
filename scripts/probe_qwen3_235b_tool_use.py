"""Probe: does qwen3-235b-a22b-instruct-2507 on DeepInfra support
multi-turn tool use well enough for the host_prep pre-interview loop?

Scope: this is a narrow, single-candidate version of BleakHouse-dbok
(the full provider×model tool-use survey). We test only what we need
to flip Phase 2.5a (pre_interview_with_refs) to the dll8-panel-winning
model, on the hosting we already use everywhere else.
...
USAGE:
    uv run python scripts/probe_qwen3_235b_tool_use.py

Requires DEEPINFRA_API_KEY in .env (already needed elsewhere in the
pipeline). Writes a one-page summary to docs/qwen3_235b_tool_use_probe.md.
"""
...
REPORT_PATH = REPO_ROOT / "docs" / "qwen3_235b_tool_use_probe.md"
...
def _write_report(probe_a: dict, probe_b: dict) -> None:
    ...
    lines = [
        f"# Tool-use probe: {MODEL} on DeepInfra",
        "",
        f"Date: 2026-05-17. Bypasses the seam; calls the openai-compat endpoint directly.",
        f"",
...
def main() -> int:
    ...
    if not api_key:
        print("ERROR: DEEPINFRA_API_KEY not set (checked .env and shell env).", file=sys.stderr)
        return 2
